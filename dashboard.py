...
with st.sidebar:
    ...
    st.markdown(
        f"{'🟢' if _av_ok else '🔴'} Alpha Vantage {'✓ Active' if _av_ok else '✗ Not set'}"
    )
    st.markdown(
        f"{'🟢' if _fh_ok else '🟡'} Finnhub {'✓ Active' if _fh_ok else 'Optional'}"
    )
    if not _av_ok:
        st.caption("Add your AV key to config.yaml → alpha_vantage_key")
